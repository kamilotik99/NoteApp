...
class MutableTextInput(FloatLayout):
    ...
    def prepare(self, *args):
        self.w_textinput = self.ids.w_textinput
        self.w_label = self.ids.w_label
        self.view()

    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos) and touch.is_touch:
            self.edit()
    ...
    def view(self):
        self.clear_widgets()
        if not self.text:
            self.w_label.text = "Введите текст заметки"
...
class NoteApp(App):
    ...
    def add_note(self):
        self.notes.data.append({'title': 'Новая заметка', 'content': ''})
